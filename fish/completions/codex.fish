...
complete -c codex -n "__fish_codex_needs_command" -s a -l ask-for-approval -d 'Configure when the model requires human approval before executing a command' -r -f -a "on-request\t'The model decides when to ask the user for approval'
...
complete -c codex -n "__fish_codex_needs_command" -a "agents" -d 'Browse all agent sessions on the shared local app-server daemon'
...
complete -c codex -n "__fish_codex_needs_command" -a "queue" -d 'Queue a message for an existing session'
...
complete -c codex -n "__fish_codex_needs_command" -a "migrate-rollouts" -d 'Inspect or migrate legacy local sessions to paginated thread history'
...
complete -c codex -n "__fish_codex_using_subcommand agents" -l remote -d 'Connect the TUI to a remote app server endpoint' -r
complete -c codex -n "__fish_codex_using_subcommand agents" -l remote-auth-token-env -d 'Name of the environment variable containing the bearer token to send to a remote app server websocket' -r
complete -c codex -n "__fish_codex_using_subcommand agents" -s C -l cd -d 'Use this directory for new tasks on a remote server' -r -F
complete -c codex -n "__fish_codex_using_subcommand agents" -s c -l config -d 'Override a configuration value that would otherwise be loaded from `~/.codex/config.toml`. Use a dotted path (`foo.bar.baz`) to override nested values. The `value` portion is parsed as TOML. If it fails to parse as TOML, the raw string is used as a literal' -r
complete -c codex -n "__fish_codex_using_subcommand agents" -l enable -d 'Enable a feature (repeatable). Equivalent to `-c features.<name>=true`' -r
complete -c codex -n "__fish_codex_using_subcommand agents" -l disable -d 'Disable a feature (repeatable). Equivalent to `-c features.<name>=false`' -r
complete -c codex -n "__fish_codex_using_subcommand agents" -l no-alt-screen -d 'Disable alternate screen mode'
complete -c codex -n "__fish_codex_using_subcommand agents" -s h -l help -d 'Print help (see more with \'--help\')'
...
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -s p -l profile -d 'Layer $CODEX_HOME/<name>.config.toml on top of the base user config' -r
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -s s -l sandbox -d 'Select the sandbox policy to use when executing model-generated shell commands' -r -f -a "read-only\t''
workspace-write\t''
danger-full-access\t''"
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -s C -l cd -d 'Tell the agent to use the specified directory as its working root' -r -F
...
never\t''
auto\t''"
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -s o -l output-last-message -d 'Specifies file where the last message from the agent should be written' -r -F
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -s c -l config -d 'Override a configuration value that would otherwise be loaded from `~/.codex/config.toml`. Use a dotted path (`foo.bar.baz`) to override nested values. The `value` portion is parsed as TOML. If it fails to parse as TOML, the raw string is used as a literal' -r
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -l enable -d 'Enable a feature (repeatable). Equivalent to `-c features.<name>=true`' -r
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -l disable -d 'Disable a feature (repeatable). Equivalent to `-c features.<name>=false`' -r
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -l strict-config -d 'Error out when config.toml contains fields that are not recognized by this version of Codex'
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -l oss -d 'Use open-source provider'
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -l approve-for-me -d 'Route approval requests through automatic review using the workspace-write sandbox'
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -l dangerously-bypass-approvals-and-sandbox -d 'Skip all confirmation prompts and execute commands without sandboxing. EXTREMELY DANGEROUS. Intended solely for running in environments that are externally sandboxed'
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -l dangerously-bypass-hook-trust -d 'Run enabled hooks without requiring persisted hook trust for this invocation. DANGEROUS. Intended only for automation that already vets hook sources'
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -l skip-git-repo-check -d 'Allow running Codex outside a Git repository'
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -l ephemeral -d 'Run without persisting session files to disk'
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -l ignore-user-config -d 'Do not load `$CODEX_HOME/config.toml`; auth still uses `CODEX_HOME`'
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -l ignore-rules -d 'Do not load user or project execpolicy `.rules` files'
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -l json -d 'Print events to stdout as JSONL'
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -s h -l help -d 'Print help (see more with \'--help\')'
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -s V -l version -d 'Print version'
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -a "resume" -d 'Resume a previous session by id or pick the most recent with --last'
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -a "fork" -d 'Fork a previous session by id into a new session'
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -a "review" -d 'Run a code review against the current repository'
complete -c codex -n "__fish_codex_using_subcommand exec; and not __fish_seen_subcommand_from resume fork review help" -a "help" -d 'Print this message or the help of the given subcommand(s)'
...
complete -c codex -n "__fish_codex_using_subcommand exec; and __fish_seen_subcommand_from fork" -l enable -d 'Enable a feature (repeatable). Equivalent to `-c features.<name>=true`' -r
complete -c codex -n "__fish_codex_using_subcommand exec; and __fish_seen_subcommand_from fork" -l disable -d 'Disable a feature (repeatable). Equivalent to `-c features.<name>=false`' -r
complete -c codex -n "__fish_codex_using_subcommand exec; and __fish_seen_subcommand_from fork" -l strict-config -d 'Error out when config.toml contains fields that are not recognized by this version of Codex'
complete -c codex -n "__fish_codex_using_subcommand exec; and __fish_seen_subcommand_from fork" -l dangerously-bypass-approvals-and-sandbox -d 'Skip all confirmation prompts and execute commands without sandboxing. EXTREMELY DANGEROUS. Intended solely for running in environments that are externally sandboxed'
complete -c codex -n "__fish_codex_using_subcommand exec; and __fish_seen_subcommand_from fork" -l dangerously-bypass-hook-trust -d 'Run enabled hooks without requiring persisted hook trust for this invocation. DANGEROUS. Intended only for automation that already vets hook sources'
complete -c codex -n "__fish_codex_using_subcommand exec; and __fish_seen_subcommand_from fork" -l skip-git-repo-check -d 'Allow running Codex outside a Git repository'
complete -c codex -n "__fish_codex_using_subcommand exec; and __fish_seen_subcommand_from fork" -l ephemeral -d 'Run without persisting session files to disk'
complete -c codex -n "__fish_codex_using_subcommand exec; and __fish_seen_subcommand_from fork" -l ignore-user-config -d 'Do not load `$CODEX_HOME/config.toml`; auth still uses `CODEX_HOME`'
complete -c codex -n "__fish_codex_using_subcommand exec; and __fish_seen_subcommand_from fork" -l ignore-rules -d 'Do not load user or project execpolicy `.rules` files'
complete -c codex -n "__fish_codex_using_subcommand exec; and __fish_seen_subcommand_from fork" -l json -d 'Print events to stdout as JSONL'
complete -c codex -n "__fish_codex_using_subcommand exec; and __fish_seen_subcommand_from fork" -s h -l help -d 'Print help (see more with \'--help\')'
...
complete -c codex -n "__fish_codex_using_subcommand exec; and __fish_seen_subcommand_from help" -f -a "fork" -d 'Fork a previous session by id into a new session'
...
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -s i -l image -d 'Optional image(s) to attach to the initial prompt' -r -F
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -l local-provider -d 'Specify which local provider to use (lmstudio or ollama). If not specified with --oss, will use config default or show selection' -r
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -s p -l profile -d 'Layer $CODEX_HOME/<name>.config.toml on top of the base user config' -r
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -s s -l sandbox -d 'Select the sandbox policy to use when executing model-generated shell commands' -r -f -a "read-only\t''
workspace-write\t''
danger-full-access\t''"
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -s C -l cd -d 'Tell the agent to use the specified directory as its working root' -r -F
...
never\t''
auto\t''"
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -s o -l output-last-message -d 'Specifies file where the last message from the agent should be written' -r -F
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -s c -l config -d 'Override a configuration value that would otherwise be loaded from `~/.codex/config.toml`. Use a dotted path (`foo.bar.baz`) to override nested values. The `value` portion is parsed as TOML. If it fails to parse as TOML, the raw string is used as a literal' -r
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -l enable -d 'Enable a feature (repeatable). Equivalent to `-c features.<name>=true`' -r
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -l disable -d 'Disable a feature (repeatable). Equivalent to `-c features.<name>=false`' -r
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -l strict-config -d 'Error out when config.toml contains fields that are not recognized by this version of Codex'
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -l oss -d 'Use open-source provider'
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -l approve-for-me -d 'Route approval requests through automatic review using the workspace-write sandbox'
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -l dangerously-bypass-approvals-and-sandbox -d 'Skip all confirmation prompts and execute commands without sandboxing. EXTREMELY DANGEROUS. Intended solely for running in environments that are externally sandboxed'
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -l dangerously-bypass-hook-trust -d 'Run enabled hooks without requiring persisted hook trust for this invocation. DANGEROUS. Intended only for automation that already vets hook sources'
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -l skip-git-repo-check -d 'Allow running Codex outside a Git repository'
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -l ephemeral -d 'Run without persisting session files to disk'
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -l ignore-user-config -d 'Do not load `$CODEX_HOME/config.toml`; auth still uses `CODEX_HOME`'
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -l ignore-rules -d 'Do not load user or project execpolicy `.rules` files'
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -l json -d 'Print events to stdout as JSONL'
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -s h -l help -d 'Print help (see more with \'--help\')'
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -s V -l version -d 'Print version'
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -a "resume" -d 'Resume a previous session by id or pick the most recent with --last'
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -a "fork" -d 'Fork a previous session by id into a new session'
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -a "review" -d 'Run a code review against the current repository'
complete -c codex -n "__fish_codex_using_subcommand e; and not __fish_seen_subcommand_from resume fork review help" -a "help" -d 'Print this message or the help of the given subcommand(s)'
...
complete -c codex -n "__fish_codex_using_subcommand e; and __fish_seen_subcommand_from fork" -s c -l config -d 'Override a configuration value that would otherwise be loaded from `~/.codex/config.toml`. Use a dotted path (`foo.bar.baz`) to override nested values. The `value` portion is parsed as TOML. If it fails to parse as TOML, the raw string is used as a literal' -r
complete -c codex -n "__fish_codex_using_subcommand e; and __fish_seen_subcommand_from fork" -l enable -d 'Enable a feature (repeatable). Equivalent to `-c features.<name>=true`' -r
complete -c codex -n "__fish_codex_using_subcommand e; and __fish_seen_subcommand_from fork" -l disable -d 'Disable a feature (repeatable). Equivalent to `-c features.<name>=false`' -r
complete -c codex -n "__fish_codex_using_subcommand e; and __fish_seen_subcommand_from fork" -l strict-config -d 'Error out when config.toml contains fields that are not recognized by this version of Codex'
complete -c codex -n "__fish_codex_using_subcommand e; and __fish_seen_subcommand_from fork" -l dangerously-bypass-approvals-and-sandbox -d 'Skip all confirmation prompts and execute commands without sandboxing. EXTREMELY DANGEROUS. Intended solely for running in environments that are externally sandboxed'
complete -c codex -n "__fish_codex_using_subcommand e; and __fish_seen_subcommand_from fork" -l dangerously-bypass-hook-trust -d 'Run enabled hooks without requiring persisted hook trust for this invocation. DANGEROUS. Intended only for automation that already vets hook sources'
complete -c codex -n "__fish_codex_using_subcommand e; and __fish_seen_subcommand_from fork" -l skip-git-repo-check -d 'Allow running Codex outside a Git repository'
complete -c codex -n "__fish_codex_using_subcommand e; and __fish_seen_subcommand_from fork" -l ephemeral -d 'Run without persisting session files to disk'
complete -c codex -n "__fish_codex_using_subcommand e; and __fish_seen_subcommand_from fork" -l ignore-user-config -d 'Do not load `$CODEX_HOME/config.toml`; auth still uses `CODEX_HOME`'
complete -c codex -n "__fish_codex_using_subcommand e; and __fish_seen_subcommand_from fork" -l ignore-rules -d 'Do not load user or project execpolicy `.rules` files'
complete -c codex -n "__fish_codex_using_subcommand e; and __fish_seen_subcommand_from fork" -l json -d 'Print events to stdout as JSONL'
complete -c codex -n "__fish_codex_using_subcommand e; and __fish_seen_subcommand_from fork" -s h -l help -d 'Print help (see more with \'--help\')'
...
complete -c codex -n "__fish_codex_using_subcommand e; and __fish_seen_subcommand_from help" -f -a "fork" -d 'Fork a previous session by id into a new session'
...
complete -c codex -n "__fish_codex_using_subcommand mcp; and __fish_seen_subcommand_from login" -l oauth-client-registration -d 'OAuth client-registration strategy for this login only' -r -f -a "auto\t''
cimd\t''
dcr\t''"
...
complete -c codex -n "__fish_codex_using_subcommand resume" -s a -l ask-for-approval -d 'Configure when the model requires human approval before executing a command' -r -f -a "on-request\t'The model decides when to ask the user for approval'
never\t'Never ask for user approval Execution failures are immediately returned to the model'"
...
complete -c codex -n "__fish_codex_using_subcommand queue" -l thread -d 'Session UUID or exact session name' -r
complete -c codex -n "__fish_codex_using_subcommand queue" -l message -d 'Message text to queue' -r
complete -c codex -n "__fish_codex_using_subcommand queue" -l remote -d 'Connect the TUI to a remote app server endpoint' -r
complete -c codex -n "__fish_codex_using_subcommand queue" -l remote-auth-token-env -d 'Name of the environment variable containing the bearer token to send to a remote app server websocket' -r
complete -c codex -n "__fish_codex_using_subcommand queue" -s i -l image -d 'Optional image(s) to attach to the initial prompt' -r -F
complete -c codex -n "__fish_codex_using_subcommand queue" -s m -l model -d 'Model the agent should use' -r
complete -c codex -n "__fish_codex_using_subcommand queue" -l local-provider -d 'Specify which local provider to use (lmstudio or ollama). If not specified with --oss, will use config default or show selection' -r
complete -c codex -n "__fish_codex_using_subcommand queue" -s p -l profile -d 'Layer $CODEX_HOME/<name>.config.toml on top of the base user config' -r
complete -c codex -n "__fish_codex_using_subcommand queue" -s s -l sandbox -d 'Select the sandbox policy to use when executing model-generated shell commands' -r -f -a "read-only\t''
workspace-write\t''
danger-full-access\t''"
complete -c codex -n "__fish_codex_using_subcommand queue" -s C -l cd -d 'Tell the agent to use the specified directory as its working root' -r -F
complete -c codex -n "__fish_codex_using_subcommand queue" -l add-dir -d 'Additional directories that should be writable alongside the primary workspace' -r -f -a "(__fish_complete_directories)"
complete -c codex -n "__fish_codex_using_subcommand queue" -s c -l config -d 'Override a configuration value that would otherwise be loaded from `~/.codex/config.toml`. Use a dotted path (`foo.bar.baz`) to override nested values. The `value` portion is parsed as TOML. If it fails to parse as TOML, the raw string is used as a literal' -r
complete -c codex -n "__fish_codex_using_subcommand queue" -l enable -d 'Enable a feature (repeatable). Equivalent to `-c features.<name>=true`' -r
complete -c codex -n "__fish_codex_using_subcommand queue" -l disable -d 'Disable a feature (repeatable). Equivalent to `-c features.<name>=false`' -r
complete -c codex -n "__fish_codex_using_subcommand queue" -l oss -d 'Use open-source provider'
complete -c codex -n "__fish_codex_using_subcommand queue" -l approve-for-me -d 'Route approval requests through automatic review using the workspace-write sandbox'
complete -c codex -n "__fish_codex_using_subcommand queue" -l dangerously-bypass-approvals-and-sandbox -d 'Skip all confirmation prompts and execute commands without sandboxing. EXTREMELY DANGEROUS. Intended solely for running in environments that are externally sandboxed'
complete -c codex -n "__fish_codex_using_subcommand queue" -l dangerously-bypass-hook-trust -d 'Run enabled hooks without requiring persisted hook trust for this invocation. DANGEROUS. Intended only for automation that already vets hook sources'
complete -c codex -n "__fish_codex_using_subcommand queue" -l strict-config -d 'Error out when config.toml contains fields that are not recognized by this version of Codex'
complete -c codex -n "__fish_codex_using_subcommand queue" -s h -l help -d 'Print help (see more with \'--help\')'
...
complete -c codex -n "__fish_codex_using_subcommand migrate-rollouts" -l thread -d 'Restrict inspection or migration to one or more thread IDs' -r
complete -c codex -n "__fish_codex_using_subcommand migrate-rollouts" -l max-mib-per-second -d 'Limit aggregate rollout read and write throughput, in MiB per second' -r
complete -c codex -n "__fish_codex_using_subcommand migrate-rollouts" -s c -l config -d 'Override a configuration value that would otherwise be loaded from `~/.codex/config.toml`. Use a dotted path (`foo.bar.baz`) to override nested values. The `value` portion is parsed as TOML. If it fails to parse as TOML, the raw string is used as a literal' -r
complete -c codex -n "__fish_codex_using_subcommand migrate-rollouts" -l enable -d 'Enable a feature (repeatable). Equivalent to `-c features.<name>=true`' -r
complete -c codex -n "__fish_codex_using_subcommand migrate-rollouts" -l disable -d 'Disable a feature (repeatable). Equivalent to `-c features.<name>=false`' -r
complete -c codex -n "__fish_codex_using_subcommand migrate-rollouts" -l apply -d 'Publish the migration. Without this flag the command only reports eligible sessions'
complete -c codex -n "__fish_codex_using_subcommand migrate-rollouts" -l json -d 'Emit the complete per-thread report as JSON'
complete -c codex -n "__fish_codex_using_subcommand migrate-rollouts" -l verbose -d 'Print one line for every inspected rollout'
complete -c codex -n "__fish_codex_using_subcommand migrate-rollouts" -s h -l help -d 'Print help (see more with \'--help\')'
...
complete -c codex -n "__fish_codex_using_subcommand fork" -s a -l ask-for-approval -d 'Configure when the model requires human approval before executing a command' -r -f -a "on-request\t'The model decides when to ask the user for approval'
never\t'Never ask for user approval Execution failures are immediately returned to the model'"
...
complete -c codex -n "__fish_codex_using_subcommand exec-server; and not __fish_seen_subcommand_from forward help" -l environment-id -d 'Environment id to attach to when registering remotely' -r
complete -c codex -n "__fish_codex_using_subcommand exec-server; and not __fish_seen_subcommand_from forward help" -l name -d 'Human-readable environment name' -r
complete -c codex -n "__fish_codex_using_subcommand exec-server; and not __fish_seen_subcommand_from forward help" -s c -l config -d 'Override a configuration value that would otherwise be loaded from `~/.codex/config.toml`. Use a dotted path (`foo.bar.baz`) to override nested values. The `value` portion is parsed as TOML. If it fails to parse as TOML, the raw string is used as a literal' -r
complete -c codex -n "__fish_codex_using_subcommand exec-server; and not __fish_seen_subcommand_from forward help" -l enable -d 'Enable a feature (repeatable). Equivalent to `-c features.<name>=true`' -r
complete -c codex -n "__fish_codex_using_subcommand exec-server; and not __fish_seen_subcommand_from forward help" -l disable -d 'Disable a feature (repeatable). Equivalent to `-c features.<name>=false`' -r
complete -c codex -n "__fish_codex_using_subcommand exec-server; and not __fish_seen_subcommand_from forward help" -l strict-config -d 'Error out when config.toml contains fields that are not recognized by this version of Codex'
complete -c codex -n "__fish_codex_using_subcommand exec-server; and not __fish_seen_subcommand_from forward help" -l use-agent-identity-auth -d 'Use Agent Identity auth from CODEX_ACCESS_TOKEN for remote registration'
complete -c codex -n "__fish_codex_using_subcommand exec-server; and not __fish_seen_subcommand_from forward help" -l exit-on-stdin-close -d 'Exit when the parent-owned standard-input pipe closes'
complete -c codex -n "__fish_codex_using_subcommand exec-server; and not __fish_seen_subcommand_from forward help" -s h -l help -d 'Print help (see more with \'--help\')'
...
complete -c codex -n "__fish_codex_using_subcommand exec-server; and __fish_seen_subcommand_from forward" -l environment-id -d 'Environment id to attach to when registering remotely' -r
complete -c codex -n "__fish_codex_using_subcommand exec-server; and __fish_seen_subcommand_from forward" -l name -d 'Human-readable environment name' -r
complete -c codex -n "__fish_codex_using_subcommand exec-server; and __fish_seen_subcommand_from forward" -s c -l config -d 'Override a configuration value that would otherwise be loaded from `~/.codex/config.toml`. Use a dotted path (`foo.bar.baz`) to override nested values. The `value` portion is parsed as TOML. If it fails to parse as TOML, the raw string is used as a literal' -r
complete -c codex -n "__fish_codex_using_subcommand exec-server; and __fish_seen_subcommand_from forward" -l enable -d 'Enable a feature (repeatable). Equivalent to `-c features.<name>=true`' -r
complete -c codex -n "__fish_codex_using_subcommand exec-server; and __fish_seen_subcommand_from forward" -l disable -d 'Disable a feature (repeatable). Equivalent to `-c features.<name>=false`' -r
complete -c codex -n "__fish_codex_using_subcommand exec-server; and __fish_seen_subcommand_from forward" -l strict-config -d 'Error out when config.toml contains fields that are not recognized by this version of Codex'
complete -c codex -n "__fish_codex_using_subcommand exec-server; and __fish_seen_subcommand_from forward" -l use-agent-identity-auth -d 'Use Agent Identity auth from CODEX_ACCESS_TOKEN for remote registration'
complete -c codex -n "__fish_codex_using_subcommand exec-server; and __fish_seen_subcommand_from forward" -l exit-on-stdin-close -d 'Exit when the parent-owned standard-input pipe closes'
complete -c codex -n "__fish_codex_using_subcommand exec-server; and __fish_seen_subcommand_from forward" -s h -l help -d 'Print help (see more with \'--help\')'
complete -c codex -n "__fish_codex_using_subcommand exec-server; and __fish_seen_subcommand_from help" -f -a "forward" -d 'Register an existing WebSocket exec-server as a remote environment'
complete -c codex -n "__fish_codex_using_subcommand exec-server; and __fish_seen_subcommand_from help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
...
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "agents" -d 'Browse all agent sessions on the shared local app-server daemon'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "exec" -d 'Run Codex non-interactively'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "review" -d 'Run a code review non-interactively'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "login" -d 'Manage login'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "logout" -d 'Remove stored authentication credentials'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "mcp" -d 'Manage external MCP servers for Codex'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "plugin" -d 'Manage Codex plugins'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "mcp-server" -d 'Start Codex as an MCP server (stdio)'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "app-server" -d '[experimental] Run the app server or related tooling'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "remote-control" -d '[experimental] Manage the app-server daemon with remote control enabled'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "completion" -d 'Generate shell completion scripts'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "update" -d 'Update Codex to the latest version'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "doctor" -d 'Diagnose local Codex installation, config, auth, and runtime health'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "sandbox" -d 'Run commands within a Codex-provided sandbox'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "debug" -d 'Debugging tools'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "execpolicy" -d 'Execpolicy tooling'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "apply" -d 'Apply the latest diff produced by Codex agent as a `git apply` to your local working tree'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "resume" -d 'Resume a previous interactive session (picker by default; use --last to continue the most recent)'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "queue" -d 'Queue a message for an existing session'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "archive" -d 'Archive a saved session by id or session name'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "delete" -d 'Permanently delete a saved session by id or session name'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "migrate-rollouts" -d 'Inspect or migrate legacy local sessions to paginated thread history'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "unarchive" -d 'Unarchive a saved session by id or session name'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "fork" -d 'Fork a previous interactive session (picker by default; use --last to fork the most recent)'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "cloud" -d '[EXPERIMENTAL] Browse tasks from Codex Cloud and apply changes locally'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "responses-api-proxy" -d 'Internal: run the responses API proxy'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "stdio-to-uds" -d 'Internal: relay stdio to a Unix domain socket'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "exec-server" -d '[EXPERIMENTAL] Run the standalone exec-server service'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "features" -d 'Inspect feature flags'
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from agents exec review login logout mcp plugin mcp-server app-server remote-control completion update doctor sandbox debug execpolicy apply resume queue archive delete migrate-rollouts unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
...
complete -c codex -n "__fish_codex_using_subcommand help; and __fish_seen_subcommand_from exec" -f -a "fork" -d 'Fork a previous session by id into a new session'
...
complete -c codex -n "__fish_codex_using_subcommand help; and __fish_seen_subcommand_from exec-server" -f -a "forward" -d 'Register an existing WebSocket exec-server as a remote environment'
