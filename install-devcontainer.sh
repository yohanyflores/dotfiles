...
install_latest_neovim() {
  ...
  if command -v nvim >/dev/null 2>&1; then
    if nvim --version </dev/null 2>&1 | grep -F "$expected_version" >/dev/null; then
      log_info "Neovim ya está en ~/.local/bin con la versión $expected_version. Saltando."
      return 0
    fi
    log_info "Versión antigua o incorrecta de Neovim detectada. Actualizando a $expected_version..."
  fi
  ...
  if [[ "$arch" == "x86_64" ]]; then
    ...
    log_warn "Arquitectura no soportada para descargar Neovim. Se usará el del sistema."
    return 1
  fi

  local nvim_tar="nvim-linux-${asset_arch}.tar.gz"
  local cache_tool_dir="${CACHE_DIR}/${name}/${version_no_v}/${os}-${arch}"
  local cache_file="${cache_tool_dir}/${nvim_tar}"
  local from_cache=false

  # 2. Buscar en caché o descargar
  if [[ -f "$cache_file" ]]; then
    log_info "Neovim encontrado en caché local del host: $cache_file"
    from_cache=true
  else
    log_info "Neovim no está en caché. Descargando desde GitHub..."
    mkdir -p "$cache_tool_dir"
    local download_url="https://github.com/neovim/neovim/releases/download/${expected_version}/${nvim_tar}"
    ...
    if ! curl "${curl_opts[@]}" -o "$cache_file" "$download_url"; then
      log_warn "Fallo al descargar Neovim desde $download_url"
      rm -f "$cache_file"
      return 1
    fi
    log_info "Neovim descargado exitosamente."
  fi
  ...
  log_info "Extrayendo Neovim en ~/.local..."
  mkdir -p "$HOME/.local"
  if tar -xzf "$cache_file" --strip-components=1 -C "$HOME/.local"; then
    ...
    log_error "Error al extraer el archivo de Neovim."
    rm -rf "$tmp_extract_dir"
    return 1
  fi
}
...
# Instalar versión moderna de Neovim superpuesta en ~/.local
install_latest_neovim
...
# ==============================================================================
# 2.5. PERSISTENCIA DE CONFIGURACIONES DE AGENTES (AGY Y OPENCODE)
# ==============================================================================
